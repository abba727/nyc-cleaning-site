    ...
    <AdminAuthShell eyebrow="Invitation" title="Create your CMS account" description="Enter the six-digit code from your invitation email. Codes are single-use and expire after 10 minutes.">
      <form className="space-y-5" onSubmit={submit}>
        ...
        <div className="space-y-2">
          ...
          <CmsVerificationCodeInput id="invite-code" value={code} onChange={setCode} describedBy="invite-code-help" />
          <p id="invite-code-help" className="text-xs leading-5 text-slate-500">Enter the latest code sent to this email address. You can paste all six digits at once.</p>
        </div>
        ...
        <div className="space-y-2"><Label htmlFor="invite-password">Password</Label><Input id="invite-password" type="password" autoComplete="new-password" value={password} onChange={event => setPassword(event.target.value)} minLength={CMS_PASSWORD_MIN_LENGTH} required /><p className="text-xs leading-5 text-slate-500">{CMS_PASSWORD_REQUIREMENT}</p></div>
        <div className="space-y-2"><Label htmlFor="invite-confirmation">Confirm password</Label><Input id="invite-confirmation" type="password" autoComplete="new-password" value={confirmation} onChange={event => setConfirmation(event.target.value)} minLength={CMS_PASSWORD_MIN_LENGTH} required /></div>
