    ...
    <section className="admin-workspace space-y-8">
      ...
      <article className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-2 border-b border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <div><h2 className="text-base font-semibold text-slate-900">Service locations</h2><p className="mt-0.5 text-xs text-slate-600">A compact, paginated table for visibility, edits, and bulk removal.</p></div>
          <span className="inline-flex w-fit items-center rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700">{locationCount} records</span>
        </div>
        ...
        {!locations.isLoading && !locations.error && locationCount > 0 ? <>
          <div className="flex flex-col gap-2 border-b border-slate-100 px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            <p className="text-xs text-slate-600">Showing <strong className="font-semibold text-slate-800">{locationRangeStart}–{locationRangeEnd}</strong> of <strong className="font-semibold text-slate-800">{locationCount}</strong> locations.</p>
            <div className="flex flex-wrap gap-2">
              <Button type="button" variant="outline" size="sm" className="h-8 px-2 text-xs" onClick={toggleCurrentPageSelection} disabled={removeManyMutation.isPending}>{allCurrentPageSelected ? "Clear page" : "Select page"}</Button>
              <Button type="button" variant="outline" size="sm" className="h-8 px-2 text-xs" onClick={() => setSelectedLocationIds(new Set((locations.data ?? []).map(location => location.id)))} disabled={removeManyMutation.isPending || selectedLocationIds.size === locationCount}>Select all {locationCount}</Button>
              {selectedLocationIds.size > 0 ? <Button type="button" variant="outline" size="sm" className="h-8 px-2 text-xs" onClick={() => setSelectedLocationIds(new Set())} disabled={removeManyMutation.isPending}>Clear</Button> : null}
              <Button type="button" variant="outline" size="sm" className="h-8 border-red-200 px-2 text-xs text-red-700 hover:bg-red-50 hover:text-red-800" onClick={removeSelectedLocations} disabled={selectedLocationIds.size === 0 || removeManyMutation.isPending}>{removeManyMutation.isPending ? <Loader2 className="spin" /> : <Trash2 className="size-3.5" />} Remove {selectedLocationIds.size > 0 ? `${selectedLocationIds.size} selected` : "selected"}</Button>
            </div>
          </div>
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-slate-200 bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500"><tr><th scope="col" className="w-10 px-4 py-2"><span className="sr-only">Select</span></th><th scope="col" className="px-3 py-2 font-semibold">Property / address</th><th scope="col" className="whitespace-nowrap px-3 py-2 font-semibold">Map</th><th scope="col" className="whitespace-nowrap px-3 py-2 font-semibold">Added</th><th scope="col" className="px-4 py-2 text-right font-semibold">Actions</th></tr></thead>
              <tbody className="divide-y divide-slate-100">{paginatedLocations.map(location => editingId === location.id ? <tr key={location.id} className="bg-slate-50/70"><td colSpan={5} className="p-3"><form className="grid gap-2 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_80px_100px_auto]" onSubmit={event => { event.preventDefault(); updateMutation.mutate({ id: location.id, ...editingLocation, label: editingLocation.label || undefined, latitude: location.latitude, longitude: location.longitude, isActive: location.isActive }); }}><Input className="h-9" value={editingLocation.address} onChange={event => setEditingLocation(current => ({ ...current, address: event.target.value }))} aria-label="Street address" required maxLength={512} /><Input className="h-9" value={editingLocation.city} onChange={event => setEditingLocation(current => ({ ...current, city: event.target.value }))} aria-label="City" required maxLength={160} /><Input className="h-9" value={editingLocation.state} onChange={event => setEditingLocation(current => ({ ...current, state: event.target.value }))} aria-label="State" required maxLength={64} /><Input className="h-9" value={editingLocation.zip} onChange={event => setEditingLocation(current => ({ ...current, zip: event.target.value }))} aria-label="ZIP code" required maxLength={24} /><div className="flex gap-2"><Button className="h-9 px-3 text-xs" type="submit" disabled={updateMutation.isPending}>{updateMutation.isPending ? <Loader2 className="spin" /> : <CheckCircle2 className="size-3.5" />} Save</Button><Button className="h-9 px-3 text-xs" type="button" variant="outline" onClick={() => setEditingId(null)} disabled={updateMutation.isPending}>Cancel</Button></div><label className="grid gap-1 text-xs font-medium text-slate-700 xl:col-span-4"><span>Internal label <em className="font-normal text-slate-500">(optional)</em></span><Input className="h-9" value={editingLocation.label} onChange={event => setEditingLocation(current => ({ ...current, label: event.target.value }))} maxLength={255} /></label></form></td></tr> : <tr key={location.id} className="hover:bg-slate-50/80"><td className="px-4 py-2.5 align-top"><input type="checkbox" className="size-4 rounded border-slate-300 text-slate-900 focus:ring-slate-500" checked={selectedLocationIds.has(location.id)} onChange={() => toggleSelectedLocation(location.id)} disabled={removeManyMutation.isPending} aria-label={`Select ${fullAddress(location)}`} /></td><td className="min-w-[300px] px-3 py-2.5 align-top"><p className="truncate font-medium leading-5 text-slate-900">{location.label || "Service location"}</p><p className="truncate text-xs leading-5 text-slate-600">{fullAddress(location)}</p></td><td className="whitespace-nowrap px-3 py-2.5 align-top"><span className={location.isActive ? "rounded-full bg-emerald-100 px-2 py-0.5 text-[11px] font-semibold text-emerald-800" : "rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-semibold text-slate-600"}>{location.isActive ? "Visible" : "Hidden"}</span></td><td className="whitespace-nowrap px-3 py-2.5 align-top text-xs text-slate-500">{formatDate(location.createdAt)}</td><td className="whitespace-nowrap px-4 py-2.5 align-top text-right"><div className="inline-flex gap-1"><Button type="button" variant="outline" size="sm" className="h-8 px-2 text-xs" onClick={() => toggleActive(location)} disabled={updateMutation.isPending || removeManyMutation.isPending}>{location.isActive ? <EyeOff className="size-3.5" /> : <Eye className="size-3.5" />}{location.isActive ? "Hide" : "Show"}</Button><Button type="button" variant="outline" size="sm" className="h-8 px-2 text-xs" onClick={() => startEditing(location)} disabled={updateMutation.isPending || removeMutation.isPending || removeManyMutation.isPending}><Pencil className="size-3.5" /> Edit</Button><Button type="button" variant="outline" size="sm" className="h-8 border-red-200 px-2 text-xs text-red-700 hover:bg-red-50 hover:text-red-800" onClick={() => { if (window.confirm(`Remove ${fullAddress(location)}? This cannot be undone.`)) removeMutation.mutate({ id: location.id }); }} disabled={removeMutation.isPending || removeManyMutation.isPending || updateMutation.isPending}><Trash2 className="size-3.5" /> Remove</Button></div></td></tr>)}</tbody>
            </table>
          </div>
          <div className="flex flex-col gap-2 border-t border-slate-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2"><Button type="button" variant="outline" size="sm" className="h-8 px-2 text-xs" onClick={() => setLocationPage(page => Math.max(1, page - 1))} disabled={currentLocationPage === 1}>Previous</Button><Button type="button" variant="outline" size="sm" className="h-8 px-2 text-xs" onClick={() => setLocationPage(page => Math.min(locationPageCount, page + 1))} disabled={currentLocationPage === locationPageCount}>Next</Button></div>
          </div>
        </> : null}
      </article>
