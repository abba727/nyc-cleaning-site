...
import { company, featuredServices, getPageByPath, getPageImage, homepageServices, normalizePath, pageParagraphs, serviceName, siteOrigin, type SitePage } from "@/content/site";
...
function LegacyRoute({ path }: { path: string }) {
  return <Suspense fallback={<section className="section not-found"><div className="container"><p className="eyebrow">Loading</p><h1>Opening article…</h1></div></section>}><LegacyContentPage path={path} /></Suspense>;
}

export function PublicPage() {
  const [location] = useLocation();
  const page = getPageByPath(location);
  if (!page) return <LegacyRoute path={location} />;
