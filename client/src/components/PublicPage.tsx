...
function HomePage({ page, initialInsights }: { page: SitePage; initialInsights?: InitialPublishedArticle[] }) {
  ...
  return <>
    <section className="home-hero">
      ...
      <div className="container hero-content"><p className="eyebrow light">Cleaning • Maintenance • Staffing</p><h1>Cleaner buildings. Reliable property care.</h1><p>Full-service cleaning and maintenance for commercial and residential properties across New York City.</p><div className="button-row"><QuoteCta className="button button-gold">Get a Free Quote</QuoteCta><Link href="/cleaning-service-nyc/" className="button button-outline-light">Explore Services</Link></div></div>
...
function InteriorHero({ page }: { page: SitePage }) {
  ...
  return <section className={isLegal ? "interior-hero legal" : "interior-hero"}><div className="container interior-hero-grid"><div><p className="eyebrow light">{page.kind === "service" ? "Professional property services" : "NYC Cleaning and Maintenance"}</p><h1>{displayTitle}</h1><p>{page.description}</p>{!isLegal && <div className="button-row"><QuoteCta className="button button-gold">Request a Quote</QuoteCta><a href={`tel:${company.phoneHref}`} className="button button-outline-light">Call {company.phoneDisplay}</a></div>}</div>{!isLegal && <div className="interior-image"><ResponsiveImage src={getPageImage(page)} alt={`${displayTitle} from NYC Cleaning and Maintenance`} loading="lazy" decoding="async" /></div>}</div></section>;
}
