...
export function LegacyContentRoute({ path }: { path: string }) {
  ...
  const articleQuery = trpc.article.byPath.useQuery({ path: normalizedPath }, { enabled: !archiveRoute, retry: false });
  ...
  if (error || articleQuery.isError) return <section className="section not-found"><div className="container"><p className="eyebrow">404</p><h1>That page could not be found.</h1><p>The page may have moved, or the address may be incomplete.</p><Link href="/" className="button button-navy">Return Home</Link></div></section>;
...
export default function LegacyContentPage({ path }: { path: string }) {
  return <ClientDataProvider><LegacyContentRoute path={path} /></ClientDataProvider>;
}
