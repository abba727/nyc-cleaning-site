...
describe("critical first-paint shell", () => {
  it("inlines the public-site background, text color, and font before external font loading", () => {
    const html = readFileSync(resolve(process.cwd(), "client/index.html"), "utf8");
    const criticalStyleIndex = html.indexOf('<style id="critical-shell">');
    const externalFontIndex = html.indexOf("fonts.googleapis.com/css2");

    expect(criticalStyleIndex).toBeGreaterThan(-1);
    expect(criticalStyleIndex).toBeLessThan(externalFontIndex);
